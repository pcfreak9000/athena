...
MYNAME=AthenaPP_S30_$(date +"%Y-%m-%d_%H-%M-%S")
...
if [ "$BINAC" ]; then
    qsub -q short -l walltime=04:00:00 -l nodes=8:ppn=26 -l pmem=768mb -N $MYNAME -o $WORKDIR/LOG_ATHENA -v WORKDIR="$WORKDIR",ATHENA_CONFIG_FILE="$ATHENA_CONFIG_FILE",ATHENABIN="$ATHENABIN",TIMELIMIT_RSTFILE="$TIMELIMIT_RSTFILE" $WORKDIR/startjob.sh
else
