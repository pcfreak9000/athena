...
if [ "$BINAC" ]; then
    qsub -q short -l walltime=00:30:00 -l nodes=1:ppn=4 -l pmem=768mb -N $MYNAME -o $WORKDIR/LOG_ATHENA -v WORKDIR="$WORKDIR",ATHENA_CONFIG_FILE="$ATHENA_CONFIG_FILE",ATHENABIN="$ATHENABIN" $WORKDIR/startjob.sh
else
