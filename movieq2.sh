...
if [ "$BINAC2" ]; then
    sbatch --partition compute -t 20:00 -N 1 --ntasks-per-node=32 --mem=16g -J MOV_$MYNAME --output="$WORKDIR"/LOG_MOVIE_"$MNAME" --error="$WORKDIR"/LOG_MOVIE_"$MNAME" --export=ALL "$WORKDIR"/moviejob2.sh
